...
function runB(aTime){
    ...
    source.start(aTime); // Start B

   
    counter += bufferSeconds; // Increase total time by length of buffer in seconds
    load(bufferA); // Load the next chunk of audio into buffer A
...
function runA(aTime){
    ...
    source.connect(audioCtx.destination);
    source.start(aTime); // Start A
    
    counter += bufferSeconds; // Increase total time by length of buffer in seconds
    load(bufferB); // Load the next chunk of audio into buffer B
...
function scheduler() {
    ...
    while(nextNotetime < audioCtx.currentTime + 0.00001) 
    {
        nextNotetime += 1.0;
